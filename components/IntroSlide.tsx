...
export function IntroSlide({ view, onEnter }: IntroSlideProps) {
  if (view === "character") {
    return (
      <section
        className="flex-1 flex w-full min-h-0 overflow-hidden animate-slide-enter"
  ...
  return (
    <section
      className="flex-1 flex items-center justify-start w-full min-h-0 overflow-visible"
      aria-label="Welcome"
    >
      <div className="flex flex-col gap-[clamp(0.6rem,1.8vh,1.1rem)] max-w-[min(480px,90vw)] w-full overflow-visible py-[clamp(0.25rem,1vh,0.75rem)]">
        ...
        <div className="bg-[rgba(80,0,130,0.82)] border-2 border-black px-[1.2rem] py-[0.75rem] type-body text-[clamp(1rem,2vh,1.35rem)] leading-[1.5] text-white/90 w-fit">
          <p>
        ...
        <button
          className="w-fit bg-[#20ff00] text-black font-pixel text-[clamp(0.4rem,0.9vh,0.55rem)] tracking-[0.08em] px-5 py-2.5 border-2 border-black transition-opacity hover:opacity-85"
          type="button"
          onClick={onEnter}
        >
          Explore My World
